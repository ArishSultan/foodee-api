...
//            return dd(implode(",", $photos));
        ...
        $post = NewsFeed::create(["user_id"=>$request->user()->id, "lat"=>$request['lat'], "lng"=>$request['lng'], "content"=>$request['content'], "photos"=>(count($request->file('photos') > 0) ? $photos_string : null)]);
        if($request->has('tags')){
            $users = $request->tags;
