...
class PostController extends Controller
{
    public function index(Request $request) {
        ...
        return NewsFeed::withCount('comments')->withCount('likes')->with(['user.profile', 'comments'=>function($query) { $query->with(['user'=>function($q){$q->select('id', 'username', 'email')->with(['profile'=>function($q){$q->select('user_id', 'avatar');}]);}])->take(3); }])->orderBy('created_at', 'desc')->paginate(6);
