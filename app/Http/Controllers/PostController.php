...
class PostController extends Controller
{
    public function index(Request $request) {
        ...
        return NewsFeed::withCount('comments')->withCount('likes')->with(['user.profile', 'comments'=>function($query) { $query->with(['user.profile'=>function($q){$q->select('username', 'email', 'avatar');}])->take(3); }])->orderBy('created_at', 'desc')->paginate(6);
